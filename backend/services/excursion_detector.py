...
class ExcursionDetector:
    ...
    async def trigger_analysis(self, alert_id: str):
        """Trigger correlation analysis and RCA for the alert"""
        self.logger.info(f"Analysis triggered for alert {alert_id}")
        ...
        try:
            ...
            session_id = await session_manager.create_session(
                user_id="system",
                query=f"Analyze excursion alert for {alert.get('equipment_id')}",
                metadata={
                    "alert_id": alert_id,
            ...
            # Initialize agent state with alert context
            initial_state = AgentState(
                thread_id=session_id,
                query_reported=f"Critical excursion detected on {alert.get('equipment_id')}",
                alert_id=alert_id,
                equipment_id=alert.get("equipment_id"),
                alert_severity=alert.get("severity"),
                violations=alert.get("violations", []),
                sensor_metrics=alert.get("sensor_data", {}).get("metrics", {}),
            ...
            self.logger.info(f"Starting agent workflow for session {session_id}")
            result = await graph.ainvoke(initial_state)
            
            # Update alert with agent analysis results
                ...
                await self.alerts_collection.update_one(
                    {"_id": ObjectId(alert_id)},
                    {
                        "$set": {
                            ...
                            "correlation_analysis": result.get("correlations"),
                            "rca_hints": result.get("recommendation_text"),
                            "analysis_timestamp": datetime.utcnow(),
                            "status": "analyzed"
                        }
                    }
                )
                self.logger.info(f"Alert {alert_id} updated with agent analysis")
            
            # Also trigger traditional correlation engine for comparison
            from services.correlation_engine import CorrelationEngine
            correlation_engine = CorrelationEngine()
            correlation_result = await correlation_engine.analyze_alert(alert_id)
            ...
            await self.alerts_collection.update_one(
                ...
                {
                    "$set": {
                        "traditional_correlation": correlation_result,
                        "dual_analysis_complete": True
                    }
                }
            )
            
            self.logger.info(f"Dual analysis complete for alert {alert_id}")
            
        except Exception as e:
            self.logger.error(f"Error in trigger_analysis for alert {alert_id}: {str(e)}")
